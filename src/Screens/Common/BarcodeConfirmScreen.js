...
import { Platform, StyleSheet, CameraRoll, Dimensions, Image, Alert, TouchableOpacity, BackHandler,TouchableHighlight, View, Text, TextInput } from 'react-native';
...
class BarcodeConfirmScreen extends Component {
    ...
    async verify(BarCodeResult) {
        ...
        return await
            fetch("https://devoci.fdnycloud.org/test/fdnyintegrations/cof/getcofinfo/" + BarCodeResult, {
                method: 'GET',
                headers: {
                    "apikey": "d1257624-1749-4768-84a6-fbe526d6407f",
                },
            }).then((response) => response.json(), (err) => console.log(err)).then(json => {
                console.log(json);
                this.setState({loading:false})
                this.props.handleValidationResult(json);
                if (json.validateHolder === "YES") {
                    this.props.saveCERT(BarCodeResult);
                    this.props.navigation.navigate("IDValidated",{appMessage:json.appMessage})
                    // Valid COF case
    ...
    static navigationOptions = ({ navigation }) => ({
        headerTitle: <View style={{width:"100%",flexDirection:"row",justifyContent:"center"}}><Text  style={{color:"white",fontSize:20}}>COF Validation</Text></View>,
        ...
        headerLeft: (<HeaderBackButton title='Back'  tintColor={"white"}  backTitleVisible={Platform.OS==='ios'} onPress={() => navigation.navigate('LogIn')} />),
        // headerRight: (
    ...
    render() {
        ...
        let children =<View>
            ...
            <View style={{margin:10}}>
                <Text style={{fontSize:14}}>Please enable the network connection of your device and try again.</Text>
            </View>
        ...
        if (BarCodeResult === '') { return null }
            ...
            let show = (CERT.length !== 0 && ValidateHolder === 'YES' && BarCodeResult === CERT) ?
                (<View style={{ flex: 1, flexDirection: "column" }}>
                    ...
                    <View style={styles.textPreview}>
                        ...
                        <Text style={{ margin: 10, fontSize: 16, color: "black" }}>Welcome back. Press <Text style={{fontWeight:"bold"}}>CONFIRM</Text> to continue your work.</Text>
                    </View>
                    <View style={{ flexDirection: "row", justifyContent: "center",  }}>
                        <Button type="outline" title="RESCAN COF"  raised 
                             containerStyle={{ width:"50%"}}  
                            onPress={() => this.ReScanCOFBarcode()}></Button>
                    </View>
                    

                    <Button type="solid" title="CONFIRM" raised
...
export default connect(mapStateToProps, mapDispatchToProps)(BarcodeConfirmScreen)
