...
import { Platform, StyleSheet, CameraRoll, Dimensions, Image, Alert, Keyboard,TouchableWithoutFeedback,ActionSheetIOS,TouchableOpacity, TouchableHighlight, Modal, Picker, View, TextInput } from 'react-native';
import { RNCamera } from 'react-native-camera';
import Spinner from 'react-native-loading-spinner-overlay';
import styles from '../../Styles/styles';
import { connect } from 'react-redux';
import { ListItem, Input, Button, Text ,Overlay,Divider} from 'react-native-elements';
import RadioSelector from '../../Components/RadioSelector';
import addressCheck from '../../COF_GIS';

class AddressOverWriteScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            ClientName: '',
    ...
    static navigationOptions = () => ({
        headerTitle: <View style={{width:"100%",flexDirection:"row",justifyContent:"center"}}><Text  style={{right: "50%",color:"white",fontSize:20}}>Client's Info</Text></View>,
    ...
    async addJobToDB(cofNum,clientName,clientAddress1,clientAddress2,clientBorough,clientZip,location){
        ...
        }).then((response) => response.json(), (err) => console.log(err)).then(resp=>{this.setState({loading:false});this.props.addClient(clientAddress1,clientName,clientAddress2,clientBorough,clientZip,resp.jobId)})
    }
    ...
    async handlecheckAddress(CERT,AddressLine1,ClientName,AddressLine2,Borough,ZipCode,Location){
        ...
        if(result>500){
            ...
            this.AddClientAndNavToTask(CERT,AddressLine1,ClientName,AddressLine2,Borough,ZipCode,Location)
        }
    }
...
let boroughPicker = (Platform.OS === 'ios')? (
    <TouchableOpacity onPress={() => this.openActionSheetios()} style={{ borderBottomColor: "grey", justifyContent:"space-between",borderBottomWidth: 1, marginLeft: 10, marginRight: 10, flex:1}}>
    <Text h4 h4Style={{ fontSize: 14, color: "black",fontWeight:"bold" }}>Borough (Required)</Text>
    <View>
        {
            this.state.Borough ==='(Required)'? <Text style={{fontSize:20,marginBottom:10,color:"#C7C7CD"}}>{this.state.Borough}</Text>:<Text style={{fontSize:20,marginBottom:10}}>{this.state.Borough}</Text>
        }
    ...
    <View style={{ borderBottomColor: "grey", borderBottomWidth: 1, marginLeft: 10, marginRight: 10, flex:1 }}>
   ...
   <Picker
        ...
        const { ClientName, AddressLine1,AddressLine2,Borough,ZipCode} = this.state;
        return (
        ...
        />
                ...
                <View style={styles.textPreview}>
                    ...
                    <Input label={"Address Line 1"} labelStyle={{ color: "black", fontSize: 14 }} placeholder={"Address Line 1 (Required)"}  placeholderTextColor={"#C7C7CD"}containerStyle={{ marginBottom: 20 }} onChangeText={(text) =>this.setState({ AddressLine1: text })} />
...
<Button type="solid" title="Continue" raised
                        buttonStyle={{ width: "100%" }}
                        disabled={this.state.ClientName.length === 0 || this.state.Borough ==='(Required)' || this.state.AddressLine1.length === 0}
                        // onPress={() => this.props.navigation.navigate('PreSurvey')}
                        onPress={() => this.handlecheckAddress(CERT,AddressLine1,ClientName,AddressLine2,Borough,ZipCode,Location)}
                        //onPress={()=>console.log("add client ")}
                />
                </View>
                ...
                <Overlay isVisible={this.state.isVisible} children={FailMessage} height="30%" onBackdropPress={()=>this.setState({isVisible:!this.state.isVisible})} >
...
export default connect(mapStateToProps, mapDispatchToProps)(AddressOverWriteScreen)
